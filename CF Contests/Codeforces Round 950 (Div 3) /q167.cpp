...
int solve(int tc)
{
    ...
    vector<int> dvalues(m);
    cin>>dvalues;

    map<int,int> neededvals;
    map<int,int> availablevals;
    ...
    for(int i=0;i<n;i++){
        if (original[i] != modified[i]){
            neededvals[modified[i]]++;
        }
    }

    for(int i=0;i<m;i++){
        availablevals[dvalues[i]]++;
    }

    for (auto iter : neededvals){
        if (availablevals[iter.first]<iter.second){
            cout<<"NO"<<endl;
            answerfound = true;
            possible = false;
            break;
    ...
    if (possible){
        possible = false;
        for(int i=0;i<n;i++)
        {   if (modified[i] == dvalues[m-1]){                // if u find it in modified its done .... over ... khtm
                possible = true;
                break;
        } 
        }
