...
// QUESTION LINK : https://codeforces.com/contest/1985/problem/F
...
int solve(int tc)
{
    ...
    // use binary search - very thoughtful actually 

    int low = 0;
    int high = 1e12;             // answer is bound to lie in this search space.
    ...
    while (low<=high){
        mid = (low+high)>>1;       // fancy way of dividing by 2 right shift by one bit that reminds me, our bit manipulation is especially weak
        
        int s = 0;
        for(int i=0;i<n;i++){
            // total attack  = no of turns * power of attack;
            s += (mid/cooldown[i]) * damage[i];
